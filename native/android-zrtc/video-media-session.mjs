...
export async function runVideoMedia(worker,{device,signal,sink},camera=runCamera,remote=runRemoteVideo) {
  ...
  try {
    await Promise.all([
      guard(()=>camera(worker,{device,signal:controller.signal})),
